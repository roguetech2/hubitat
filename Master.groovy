...
def applyLabel(text){
    return '<div style="background-color:#DCDCDC"><b>' + text + ':</b></div>'
}

def applyInfo(text,noDisplayIcon = null){
    infoIcon = "<img src=\"http://emily-john.love/icons/information.png\" width=20 height=20>"
    if(noDisplayIcon) return '<div style="background-color:AliceBlue">' + text + '</div>'
    if(!noDisplayIcon) return '<div style="background-color:AliceBlue">' + infoIcon + ' ' + text + '</div>'
}
def applyError(text,noDisplayIcon = null){
    errorIcon = "<img src=\"http://emily-john.love/icons/error.png\" width=20 height=20>"
    if(noDisplayIcon) return '<div style="background-color:Bisque">' + text + '</div>'
    if(!noDisplayIcon) return '<div style="background-color:Bisque">' + errorIcon  + ' ' + text + '</div>'
}
def applyWarning(text,noDisplayIcon = null){
    errorIcon = "<img src=\"http://emily-john.love/icons/error.png\" width=20 height=20>"
    if(noDisplayIcon) return '<div style="background-color:LemonChiffon">' + text + '</div>'
    if(!noDisplayIcon) return '<div style="background-color:LemonChiffon">' + warningIcon  + ' ' + text + '</div>'
}
def applyHighlight(text){
...
def singleLock(action, singleDevice, childLabel = 'Master'){
    if(action == 'lock'){
        ...
        putLog(292,'error',"Invalid value for action \"$action\" sent to singleLock function",childLabel)
    }
    putLog(294,'info',action + "ed $singleDevice",childLabel)
}
...
def waitStateChange(action, singleDevice, childLabel = "Master"){
    ...
    if((action == "on" && deviceState == "on") || (action == "off" && deviceState == "off")) {
        putLog(314,"debug","Waited " + (loopCount * 10) + " milliseconds for $singleDevice to turn on",childLabel)
        return true
    } else {
        putLog(317,"error","$singleDevice refused to turn $action in " + (loopCount * 10) + " milliseconds",childLabel)
        return
    }
}
*/
...
def dim(action, multiDevice, appId, childLabel='Master'){
    if(action != 'dim' && action != 'brighten'){
        putLog(328,'error',"Invalid value for action \"$action\" sent to dim function",childLabel)
        return
    }
...
def nextLevel(singleDevice, action, childLabel='Master'){
    ...
    if(childLabel != 'Master'){
        ...
        if(!dimSpeed){
            dimSpeed = 1.2
            putLog(399,'error','ERROR: Failed to find dimSpeed in function nextLevel',childLabel)
        }
    }
    if (action != 'dim' && action != 'brighten'){
        putLog(403,'error',"ERROR: Invalid value for action \"$action\" sent to nextLevel function",childLabel)
        return false
    }
...
def changeMode(mode, childLabel = 'Master', device = ''){
    ...
    putLog(437,'info','Changed Mode from ' + oldMode + ' to ' + mode,childLabel)
}
...
def sendPushNotification(phone, message, childLabel = 'Master'){
    putLog(443,'info','Sent "' + message + '" to ' + phone,childLabel)
...
def isAnyOnMulti(multiDevice,childLabel='Master'){
    if(!multiDevice) return
    returnStatus = false
    multiDevice.each{singleDevice->
        if(isOn(singleDevice,childLabel)) returnStatus = true
    }
    return returnStatus
}
...
// Test state of a group of switches
// Return true if any are on
def isAllOnMulti(multiDevice,childLabel='Master'){
    if(!multiDevice) return
    returnStatus = true
    multiDevice.each{singleDevice->
        if(!isOn(singleDevice,childLabel)) returnStatus = false
    }
    return returnStatus
}
...
def validateMultiplier(value, childLabel='Master'){
    if(value){
        if(value < 1 || value > 100){
            putLog(537,'error',"ERROR: Multiplier $value is not valid",childLabel)
            return
        }
    }
    return value
...
// Need to change this to either Any or All
//Need code to track and return the result
def checkActiveScheduleMulti(multiDevice,childLabel='Master'){
    if(!multiDevice) return
    multiDevice.each{singleDevice->
        match = checkActiveScheduleSingle(singleDevice,childLabel)
    }
    return match
}

// deprecated function?
def checkActiveScheduleSingle(singleDevice,childLabel='Master'){
    if(!singleDevice) return
// Fix this to look for appIds in the table
    childApps.each {Child->
        if(Child.label.substring(0,7) == 'Time - ') {
...
def speakSingle(text,deviceId, childLabel='Master'){
    if(!deviceId) {
        putLog(721,'warn','No speech device for " ' + text + '"',childLabel)
        return
    }
    speechDevice.each{
        if(it.id == deviceId){
            it.speak(text)
            putLog(727,'info','Sending speech "' + text + '" to device ' + it,childLabel)
            return true
        }
    }
}
...
def handleFanWithStartLevel(singleDevice,defaults,childLabel = 'Master'){
    ...
    if(singleDevice.currentSpeed == atomicState."deviceData${singleDevice.id}".'level'.'startLevel'){
        ...
        putLog(1002,'debug','Turning ' + singleDevice + ' on',childLabel)
    }
    if(singleDevice.currentSpeed != defaults.'level'.'startLevel'){
        ...
        putLog(1006,'debug','Setting ' + singleDevice + ' to ' + defaults.'level'.'startLevel',childLabel)
    }
...
def handleFanWithoutStartLevel(singleDevice,defaults,childLabel = 'Master'){
    ...
    if(singleDevice.currentSpeed == 'high'){
        singleDevice.setSpeed('on')
        putLog(1015,'debug','Turning ' + singleDevice + ' on',childLabel)
        return true
    }
    singleDevice.setSpeed('high')
    putLog(1019,'debug','Setting ' + singleDevice + ' to high',childLabel)
}
...
def handleDimmableWithStartLevel(singleDevice,defaults,childLabel = 'Master'){
    ...
    if(singleDevice.currentLevel == defaults.'level'.'startLevel'){
        singleDevice.on()
        putLog(1040,'debug','Turning ' + singleDevice + ' on',childLabel)
    }
    if(singleDevice.currentLevel != defaults.'level'.'startLevel'){
        singleDevice.setLevel(defaults.'level'.'startLevel')
        putLog(1044,'debug','Setting ' + singleDevice + ' to ' + defaults.'level'.'startLevel',childLabel)
    }
    if(defaults.'level'?.'time' == 'stop') defaults.remove('level')
}
def handleDimmableWithoutStartLevel(singleDevice,defaults,childLabel = 'Master'){
    ...
    if(singleDevice.currentLevel == 100){
        singleDevice.on()
        putLog(1052,'debug','Turning ' + singleDevice + ' on',childLabel)
        return
    }
    singleDevice.setLevel(100)
    putLog(1056,'debug','Setting ' + singleDevice + ' to 100%',childLabel)
}
    ...
    if(!handleColorWithoutStartLevel(singleDevice,defaults,childLabel)){
        ...
        if(defaults.'level' && isDimmable(singleDevice,childLabel)){
            ...
            if(defaults.'level'.'startLevel' && isNumeric(defaults.'level'.'appId')){
                ...
                if(!levelStart) putLog(1103,'warn',"$singleDevice level data = " + defaults.'level' + " for schedule id " + defaults.'level'.'appId' + " but schedule isn't active",childLabel)
                // If just start level
        ...
        if(defaults.'hue'){
            ...
            if(defaults.'hue'.'startLevel' && isNumeric(defaults.'hue'.'appId')){
                ...
                if(!hueStart) putLog(1169,'warn',"$singleDevice hue data = " + defaults.'hue' + " for schedule id " + defaults.'hue'.'appId' + " but schedule isn't active",childLabel)
                // If just start hue
            } else if(defaults.'hue'.'startLevel'){
                hueStart = defaults.'hue'.'startLevel'
            } else if(!isNumeric(defaults.'hue'.'appId')){
                putLog(1174,'error',"ERROR: $singleDevice hue data = " + defaults + "; Hue node without start hue",childLabel)
            }
        ...
        if(defaults.'sat'){
            ...
            if(defaults.'sat'.'startLevel' && isNumeric(defaults.'sat'.'appId')){
                ...
                if(!satStart) putLog(1227,'warn',"$singleDevice sat data = " + defaults.'sat' + " for schedule id " + defaults.'sat'.'appId' + " but schedule isn't active",childLabel)
                // If just start sat
            } else if(defaults.'sat'.'startLevel'){
                satStart = defaults.'sat'.'startLevel'
            } else if(!isNumeric(defaults.'sat'.'appId')){
                putLog(1232,'error',"ERROR: $singleDevice sat data = $defaults; Sat node without start sat",childLabel)
            }
        ...
        if(!hueStart && !satStart && defaults.'temp'){
            ...
            if(defaults.'temp'.'startLevel' && isNumeric(defaults.'temp'.'appId')){
                ...
                if(!tempStart) putLog(1283,'warn',"$singleDevice temp data = " + defaults.'temp' + " for schedule id " + defaults.'temp'.'appId' + " but schedule isn't active",childLabel)
                // If just start temp
        ...
        if(hueStart || satStart){
            ...
            putLog(1327,'info','Set ' + singleDevice + ' ' + message,childLabel)
            if(settings['colorStaging']){
                pauseExecution(200)
                singleDevice.on()
                putLog(1331,'info','Set ' + singleDevice + ' on',childLabel)
            }
            putLog(1333,message,'info',childLabel)
        } else if(levelStart && isDimmable(singleDevice,childLabel)){
            if(tempStart){
                singleDevice.setColorTemperature(tempStart)
                putLog(1337,'info','Set ' + singleDevice + ' temperature color set to ' + tempStart + 'K',childLabel)
                pauseExecution(200)
            }
            if(singleDevice.currentLevel != levelStart){
                singleDevice.setLevel(levelStart)
                putLog(1342,'info','Set ' + singleDevice + ' brightness to ' + levelStart + '%',childLabel)
            } else {
                singleDevice.on()
                putLog(1345,'info','Turned ' + singleDevice + 'on',childLabel)
            }
        } else if(tempStart && isColor(singleDevice,childLabel)){
            singleDevice.setColorTemperature(tempStart)
            putLog(1349,'info','Set ' + singleDevice + 'temperature color set to ' + tempStart + 'K',childLabel)
            if(settings['colorStaging']){
                pauseExecution(200)
                singleDevice.on()
                putLog(1353,'info',"Set $singleDevice on",childLabel)
            }
        } else {
            if(singleDevice.currentValue('switch') != 'on') putLog(1356,'info','Set ' + singleDevice + ' on',childLabel)
            singleDevice.on()
        }
    }
    return true
...
def handleColorWithoutStartLevel(singleDevice,defaults,childLabel = 'Master'){
    ...
    if(singleDevice.currentLevel == 100){
        singleDevice.on()
        putLog(1372,'debug','Turning ' + singleDevice + ' on',childLabel)
    }
    if(singleDevice.currentLevel != 100){
        singleDevice.setLevel(100)
        putLog(1376,'info','Turning ' + singleDevice + ' on by setting to 100%',childLabel)
    }
    if(singleDevice.currentColorMode == 'RGB'){
        pauseExecution(200)
        singleDevice.currentColorMode == 'CT'
        putLog(1381,'info','Set color mode to "CT"',childLabel)
    }
    //this needs a fudge check
    if(singleDevice.currentTemperatureColor != 3500){
        pauseExecution(200)
        singleDevice.setColorTemperature(3500)
        putLog(1387,'info','Set ' + singleDevice + ' temperature color to 3500K',childLabel)
    }
...
def setStateSingle(singleDevice,childLabel = 'Master'){
    ...
    if(atomicState."deviceState${singleDevice.id}".'state' == 'off'){
        ...
        putLog(1400,'info','Turning ' + singleDevice + 'off',childLabel)
        return
    }
    ...
    if(!setStateSingleColor(singleDevice,defaults,childLabel)){
        if(singleDevice.currentValue('switch') != 'on') putLog(1409,'info','Set ' + singleDevice + ' on',childLabel)
        singleDevice.on()
        putLog(1411,'info','Turning ' + singleDevice + ' on',childLabel)
    }
    return
}
