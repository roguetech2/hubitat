...
/*
Restructure the app, including Master, to:
initializeSchedules
	schedule runDailySchedule
	[if correct time] schedule runIncrementalSchedule
maybe rename all device variables to singleDevice or mutliDevice??
maybe add data check in each function to test it?
*/
...
def initializeSchedules(){
	...
	if(state.weekDays) {
		logTrace(685,"Scheduling runDayOnSchedule for " + Date.parse("yyyy-MM-dd'T'HH:mm:ss", atomicState.start).format("h:mma MMM dd, yyyy", location.timeZone) + " (0 $minutes $hours ? * $state.weekDays)","debug")
		schedule("0 " + minutes + " " + hours + " ? * " + state.weekDays, runDayOnSchedule)
	} else {
		logTrace(688,"Scheduling runDayOnSchedule for " + Date.parse("yyyy-MM-dd'T'HH:mm:ss", atomicState.start).format("h:mma MMM dd, yyyy", location.timeZone) + " (0 $minutes $hours * * ?)","debug")
	...
	if(atomicState.stop){
		...
		if(state.weekDays) {
			logTrace(697,"Scheduling runDayOffSchedule for " + Date.parse("yyyy-MM-dd'T'HH:mm:ss", atomicState.stop).format("h:mma MMM dd, yyyy", location.timeZone) + " (0 $minutes $hours ? * $state.weekDays)","debug")
			schedule("0 " + minutes + " " + hours + " ? * " + state.weekDays, runDayOffSchedule)
		} else {
			logTrace(700,"Scheduling runDayOffSchedule for " + Date.parse("yyyy-MM-dd'T'HH:mm:ss", atomicState.stop).format("h:mma MMM dd, yyyy", location.timeZone) + " (0 $minutes $hours * * ?)","debug")
...
def incrementalSchedule(){
	...
	if(disable || state.disable) {
		logTrace(723,"Function incrementalSchedule returning; schedule disabled","debug")
		return
	}
	...
	if(ifMode && location.mode != ifMode) {
		logTrace(732,"incrementalSchedule returning, mode $ifMode","debug")
		return
    }

    if(atomicState.stop && Date.parse("yyyy-MM-dd'T'HH:mm:ss", atomicState.stop).time < new Date().time) {
        setTime()
    }
    ...
    if(parent.timeBetween(atomicState.start, atomicState.stop, app.label)){
        // Check if device(s) are on
		if(!parent.isMultiOn(timeDevice)){
			logTrace(743,"Since $timeDevice is off, stopping recurring schedules","debug")
			return
		}
...
//TO-DO: Add warning on setup page if minimum duration is too low (override it?)
		...
		logTrace(754,"Scheduling incrementalSchedule for 20 seconds","debug")
		return true
	} else {
		logTrace(757,"Schedule ended; now after $atomicState.stop","debug")
	}
}
...
Restructure the app, including Master, to:
runIncrementalSchedule
	[if correct time] setMultiLevels
...
/*
Restructure the app, including Master, to:
rename runDayOnSchedule to runDailySchedule
runDailySchedule (on or off)
	setTime
	[if toggle] get toggle state
	[if turning on or off] singleOn (each)
	[if turning on and not overriding] setMultiLevels
		[if on] getDefaults (each)
		[if on] setSingleLevel (each)
	[if turning on and not overriding] reschedule (each schedule EXCEPT this one)
		schedule runDailySchedule [on]
		schedule runDailySchedule [off]
		[if correct time] schedule runIncrementalSchedule
		runIncrementalSchedule
	[if setting levels and overriding] setSingleLevel (each)
	schedule runDayOnSchedule
*/
...
def getDefaultLevel(deviceId){
	...
	timeDevice.findAll( {it.id == deviceId} ).each {
		logTrace(849,"getDefaultLevel matched deviceId $deviceId as $it","debug")
        device = it
		match = true
    }
    ...
    if(!levelOn && !tempOn && !hueOn && !satOn){
        logTrace(861,"No starting levels set for $device","debug")
        return defaults
	}

	// If disabled, return nulls
	if(disable || state.disable) {
		logTrace(867,"Returning null level for $device, schedule disabled","debug")
		return defaults
	}
	...
	if(ifMode){
		if(location.mode != ifMode) {
			logTrace(874,"Return null level for $device, mode $ifMode","debug")
			return defaults
		}
	}
    ...
    if((levelOn && levelOff) || (tempOn && tempOff) || (hueOn && hueOff) || (satOn && satOff)){
		...
		if(!elapsedFraction) {
			logTrace(890,"Unable to calculate elapsed time with start \"$atomicState.start\" and stop \"$atomicState.stop\"","error")
			return defaults
		}
    }
	...
	logTrace(946,"Returning levels $defaults for $device","debug")
	return defaults
}
...
def setStartStopTime(type){
    if(type != "Start" && type != "Stop") {
       logTrace(959,"Invalid value for type \"$type\" sent to setStartStopTime function","error")
       return
       }
    ...
    if(settings["input${type}Type"] == "time"){
		...
		logTrace(976,"input" + type + "Type set to " + settings["input${type}Type"],"error")
		return
	}
	...
	logTrace(983,"$type time set as " + Date.parse("yyyy-MM-dd'T'HH:mm:ss", value).format("h:mma MMM dd, yyyy", location.timeZone),"trace")
...
def setWeekDays(){
	...
	logTrace(1022,"weekDaysToNum returning $dayString","debug")
...
def setTotalSeconds(){
	...
	logTrace(1037,"Schedule total seconds is $atomicState.totalSeconds","trace")
	return true
}
...
def getElapsedFraction(){
	if(!atomicState.totalSeconds) setTotalSeconds()
	...
	if(elapsedSeconds < 1){
		...
		elapsedFraction = Math.floor(elapsedSeconds / atomicState.totalSeconds * 1000) / 1000
	}

    if(elapsedFraction > 1 && !setTime()) return

	logTrace(1060,elapsedFraction * 100 + "% has elapsed in the schedule","debug")
	return elapsedFraction
}

def multiOn(action,device){
    if(!action || (action != "on" && action != "off" && action != "toggle")) {
        logTrace(1066,"Invalid action \"$action\" sent to multiOn","error")
        return
    }
